...
void Menu_initMenu(Menu_t* menu, int i)
{
	...
	switch (i) {
		...
		case MENU_INGAME_VIDEO: {
			...
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Deblur:", menu->doomRpg->doomCanvas->Deblur ? "on" : "off", 0,0);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "FPS Counter:", menu->doomRpg->doomCanvas->FPS ? "on" : "off", 0,0);
			break;
		}
		...
		case MENU_INGAME_BINDINGS: {
			...
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "D-Up", NULL, 0,0); //1 | 8, 0);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Move Back:", NULL, 0, 1);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "D-Down", NULL, 0,0); //1 | 8, 1);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Move Left:", NULL, 0, 4);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "D-Left", NULL, 0,0); //1 | 8, 4);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Move Right:", NULL, 0, 5);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "D-Right", NULL, 0,0); //1 | 8, 5);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Turn Left:", NULL, 0, 2);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "C-Left/Stick", NULL, 0,0); //1 | 8, 2);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Turn Right:", NULL, 0, 3);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "C-Right/Stick", NULL, 0,0); //1 | 8, 3);
			...
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "A", NULL, 0,0); //1 | 8, 8);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Next Weapon:", NULL, 0, 6);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "B", NULL, 0,0); //1 | 8, 6);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Prev Weapon:", NULL, 0, 7);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "", NULL, 0,0); //1 | 8, 7);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Pass Turn:", NULL, 0, 9);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Z/L", NULL, 0,0); //1 | 8, 9);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Automap:", NULL, 0, 10);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "R", NULL, 0,0); //1 | 8, 10);

			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Menu Open/Back:", NULL, 0, 11);
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Start", NULL, 0,0); //1 | 8, 11);
...
int Menu_select(Menu_t* menu, int menuId, int itemId)
{
	...
	switch (menuId)
	{
		...
		case MENU_INGAME_VIDEO:
			...
			else if(itemId == 9){ //new N64 Deblur
				menu->doomRpg->doomCanvas->Deblur ^= true;
				rdp_setdeblur(menu->doomRpg->doomCanvas->Deblur);
				strncpy(menuSystem->items[itemId].textField2, menu->doomRpg->doomCanvas->Deblur ? "on" : "off", sizeof(menuSystem->items[itemId].textField2));
			}
			else if(itemId == 10){ //FPS Counter
				menu->doomRpg->doomCanvas->FPS ^= true;
				SetFPSCounter(menu->doomRpg->doomCanvas->FPS);
				strncpy(menuSystem->items[itemId].textField2, menu->doomRpg->doomCanvas->FPS ? "on" : "off", sizeof(menuSystem->items[itemId].textField2));
			}
