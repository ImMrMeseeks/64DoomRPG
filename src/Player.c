...
{
	player->doomRpg->hud->damageTime = (player->doomRpg->doomCanvas->time + 500) % MAXTICKS;
...
boolean Player_useCollarItem(Player_t* player)
{
	...
	if (entity->def->eType != 1) {
		...
		player->doomRpg->doomCanvas->captureTime = (player->doomRpg->doomCanvas->time + 200) % MAXTICKS;
