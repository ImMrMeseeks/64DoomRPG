...
void Hud_drawBarTiles(Hud_t* hud, int x, int y, int width, boolean isLargerStatusBar)
{
    ...
    DoomCanvas_drawStatusBar(hud->doomRpg->doomCanvas, img, 0, 0, width, height, 0, x, y, 0);
}

void Hud_drawBottomBar(Hud_t* hud)
{
    testsub("Hud_drawBottomBar");
    ...
    dy = dispH - (stbH >> 1);
testsub("Hud_drawBottomBar1");
    // draw health
    //DoomCanvas_drawImageSpecial(doomCanvas, img, 0, 0, hud->iconSheetWidth, hud->iconSheetHeight, 0, hud->statusHealthXpos + cx, dy, 0x24);
    DoomCanvas_drawTile(doomCanvas, img, 0, 0, img->width, hud->iconSheetHeight, 0, hud->statusHealthXpos + cx, dy - (hud->iconSheetHeight>>1), 0);//0x24);
    ...
    DoomCanvas_drawTile(doomCanvas, img, 0, img->width, hud->iconSheetWidth, hud->iconSheetHeight, 0, hud->statusArmorXpos + cx, dy - (hud->iconSheetHeight>>1), 0);//0x24);
...
testsub("Hud_drawBottomBar3");
    ...
    DoomCanvas_drawTile(doomCanvas, &hud->imgHudFaces, 0, faceState * hud->hudFaceHeight, hud->hudFaceWidth, hud->hudFaceHeight, 0, faceX, dy - (hud->iconSheetHeight), 0);//0x24);
...
testsub("Hud_drawBottomBar4");
    ...
    if (hud->doomRpg->player->weapons) {
        ...
        if (weapon == 0) {
            ...
            DoomCanvas_drawTile(doomCanvas, img, 0, hud->iconSheetHeight << 1, hud->iconSheetWidth, hud->iconSheetHeight, 0, hud->statusAmmoXpos + cx, dy - (hud->iconSheetHeight>>1), 0);//0x24);
            strncpy(hud->ammoNum, "--", 3); //4);
            
        }
        else {
            testsub("Hud_drawBottomBar4c");
            ...
            DoomCanvas_drawTile(doomCanvas, img, 0, hud->iconSheetHeight * (combat->weaponInfo[weapon].ammoType + 3), hud->iconSheetWidth, hud->iconSheetHeight, 0, hud->statusAmmoXpos + cx, dy - (hud->iconSheetHeight>>1), 0);//0x24);
 
            SDL_snprintf(hud->ammoNum, 3, "%d", hud->doomRpg->player->ammo[combat->weaponInfo[weapon].ammoType]);
        }
        testsub("Hud_drawBottomBar4d");
        DoomCanvas_drawFont(doomCanvas, hud->ammoNum, hud->statusAmmoXpos + cx + hud->iconSheetWidth + x * 2, y, 9, 0, 2, hud->largeHud);
    }
testsub("Hud_drawBottomBar5");
    ...
    //DoomCanvas_drawTile(doomCanvas, &hud->imgStatusArrow, 0, 0, hud->imgStatusArrow.width, hud->imgStatusArrow.width, 0, hud->statusOrientationArrowXpos + cx, y - 3, 0);
...
void Hud_drawEffects(Hud_t* hud)
{
    ...
    if (doomCanvas->time < hud->damageTime) {
        ...
        if (hud->damageDir && (hud->damageCount > 0)) {
            ...
            DoomCanvas_drawTile(doomCanvas, &hud->imgAttArrow, 0, srcY, 18, 18, 0, x, y, 0x30);
        }
    }
