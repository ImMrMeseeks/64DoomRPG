...
void CombatEntity_calcDamage(DoomRPG_t* doomrpg, CombatEntity_t* ce1, Weapon_t* wpn, CombatEntity_t* ce2, int i, int dist, int* getDamage, int* getDamageArmmor)
{
	...
	//Hud_addMessage(doomrpg->hud, "calc damage"); //new
testsub("CombatEntity_calcDamage");
	...
	distance = 4096;
	distStep = 1; //2 //causing long loops??
	decDmg = (calDmg * 76) >> 8;
testsub("CombatEntity_calcDamage1");
	//causing long loops??
	int weaponRange = Weapon_getRangeMinToDist(wpn);
	//while (distance < dist - Weapon_getRangeMinToDist(wpn)) {
	while (distance < dist - weaponRange) {
		calDmg -= decDmg;
		distStep++;
		//distance = (distStep * 64) * (distStep * 64);
		distance = (distStep << 6) * (distStep << 6);
	}
	if (calDmg < 256) {
		calDmg = 256;
	}
...
int CombatEntity_calcHit(DoomRPG_t* doomrpg, CombatEntity_t* ce1, Weapon_t* wnp, CombatEntity_t* ce2, int dist) {
	...
	//Hud_addMessage(doomrpg->hud, "calc hit"); //new
	testsub("CombatEntity_calcHit");
