...
static int __validate_header( uint8_t *sector )
{
    ...
    if( (checksum != (uint16_t)((sector[current_block + 0x1C])<<8 | (sector[current_block + 0x1D]))) 
        && (checksum != 0xFFF2 - (uint16_t)(sector[current_block + 0x1E])) ) { return -1; }
    current_block = 0x60;
    checksum = __get_header_checksum((uint16_t *)&sector[current_block]);
    if( (checksum != (uint16_t)((sector[current_block + 0x1C])<<8 | (sector[current_block + 0x1D]))) 
        && (checksum != 0xFFF2 - (uint16_t)(sector[current_block + 0x1E])) ) { return -1; }
...
static int __get_free_space( uint8_t *sector )
{
    int space = 0;
	int i;
    for( i = BLOCK_VALID_FIRST; i <= BLOCK_VALID_LAST; i++ )
    {
