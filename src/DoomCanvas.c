...
//N64 New
void DoomCanvas_SaveManager(DoomCanvas_t* doomCanvas){
	...
	while (finished != true){
		...
		Hud_drawBarTiles(doomCanvas->hud, 0, 0, doomCanvas->displayRect.w, false);
		DoomCanvas_drawFont(doomCanvas, "Save Manager", 10, 5, 0, 0, strlen("Save Manager"), false);

		Hud_drawBarTiles(doomCanvas->hud, 0, 220, doomCanvas->displayRect.w, false);
		DoomCanvas_drawFont(doomCanvas, "A - Select", 10, 220, 0, 0, strlen("A - Select"), false);
		DoomCanvas_drawFont(doomCanvas, "B - Exit", 120, 220, 0, 0, strlen("B - Exit"), false);

		if((controllers & CONTROLLER_2_INSERTED) && identify_accessory(1) == ACCESSORY_MEMPAK){
		DoomCanvas_drawFont(doomCanvas, "R - Copy", 240, 220, 0, 0, strlen("A - Select"), false);
		}

		//draw lines
		DoomRPG_setColor(doomCanvas->doomRpg, 0x050A4A);
		int i3 = (doomCanvas->displayRect.h % 8) >> 1;
		int i4 = (doomCanvas->displayRect.w % 8) >> 1;
		for (i4; i4 < doomCanvas->displayRect.w; i4 += 8) {
			DoomRPG_drawLine(doomCanvas->doomRpg, i4, doomCanvas->doomRpg->hud->statusTopBarHeight, i4, (doomCanvas->displayRect.h - doomCanvas->doomRpg->hud->statusBarHeight) - 1);
		}

		int i5 = doomCanvas->doomRpg->hud->statusTopBarHeight + i3;
		for (i5; i5 < doomCanvas->displayRect.h - 20; i5 += 8) {
			DoomRPG_drawLine(doomCanvas->doomRpg, 0, i5, doomCanvas->displayRect.w, i5);
		}
		DoomRPG_setColor(doomCanvas->doomRpg, 0x000000);

		//C1
		if((controllers & CONTROLLER_1_INSERTED) && identify_accessory(0) == ACCESSORY_MEMPAK){
			freespace = get_mempak_free_space(0);
			sprintf(text, "C1 Free blocks=%i", freespace);
			DoomCanvas_drawFont(doomCanvas, text, 150, 5, 0, 0, strlen(text), false);

			DoomCanvas_drawImage(doomCanvas, &doomCanvas->menuSystem->imgHand, 20, 35 + (selected * 10), 40);
			for(i = 0; i < 16; i++){
				get_mempak_entry(0, i, &entry);
				memcpy(text, entry.name, 19);
				DoomCanvas_drawFont(doomCanvas, text, 30, 30 + (i * 10), 0, 0, 19, false);
				sprintf(text, "%i", entry.blocks);
				DoomCanvas_drawFont(doomCanvas, text, 150, 30 + (i * 10), 0, 0, 3, false);
			}
		}
		else{
			DoomCanvas_drawFont(doomCanvas, "Insert Controller1 ", 20, 80, 0, 0, 20, false);
			DoomCanvas_drawFont(doomCanvas, "with MemPak", 50, 100, 0, 0, 12, false);
		}

		//C2
		if((controllers & CONTROLLER_2_INSERTED) && identify_accessory(1) == ACCESSORY_MEMPAK){
			for(i = 0; i < 16; i++){
				get_mempak_entry(1, i, &entry);
				memcpy(text, entry.name, 19);
				DoomCanvas_drawFont(doomCanvas, text, 170, 30 + (i * 10), 0, 0, 19, false);
				sprintf(text, "%i", entry.blocks);
				DoomCanvas_drawFont(doomCanvas, text, 300, 30 + (i * 10), 0, 0, 3, false);
			}			
		}


		if(delete == true){
			SDL_SetRenderDrawColor(NULL, 0x6c, 0x6c, 0x6c, 0xff);
			SDL_RenderFillRect2(NULL, &cliprect);
			DoomCanvas_drawFont(doomCanvas, "Delete Save?", 130, 105, 0, 0, strlen("Delete Save?"), false);
			DoomCanvas_drawFont(doomCanvas, "A - Yes", 145, 120, 0, 0, strlen("A - Yes"), false);
			DoomCanvas_drawFont(doomCanvas, "B - No", 145, 130, 0, 0, strlen("B - No"), false);
			if(Down1.A){
				get_mempak_entry(0, selected, &entry);
				delete_mempak_entry(0, &entry);
				delete = false;
			}
			if(Down1.B){
				delete = false;
			...
			SDL_SetRenderDrawColor(NULL, 0x6c, 0x6c, 0x6c, 0xff);
			SDL_RenderFillRect2(NULL, &cliprect);
			DoomCanvas_drawFont(doomCanvas, "Copy Save?", 140, 105, 0, 0, strlen("Copy Save?"), false);
			DoomCanvas_drawFont(doomCanvas, "A - Yes", 145, 120, 0, 0, strlen("A - Yes"), false);
			DoomCanvas_drawFont(doomCanvas, "B - No", 145, 130, 0, 0, strlen("B - No"), false);
			if(Down1.A){
				get_mempak_entry(0, selected, &entry);
				byte* data = malloc(entry.blocks);
				read_mempak_entry_data(0, &entry, &data);
				write_mempak_entry_data(1, &entry, &data);
				free(data);
				copy = false;
			}
			if(Down1.B){
			...
			if(Down1.up || Down1.x > 20) 
				if(selected > 0) selected--;

			if(Down1.down || Down1.x < -20) 
				if(selected < 16) selected++;

			if(Down1.A) delete = true;
			if(Down1.B) finished = true;
			if(Down1.R) copy = true;
		}
...
//N64 New
void DoomCanvas_CheckPak(DoomCanvas_t* doomCanvas){
	...
	while (valid != true){
		...
		//controller inserted
		if((get_controllers_present() & CONTROLLER_1_INSERTED) != CONTROLLER_1_INSERTED){
			DoomCanvas_drawFont(doomCanvas, "No Controller", 120, 100, 0, 0, strlen("No Controller"), false);
			DoomCanvas_drawFont(doomCanvas, "Connect Player1 controller", 70, 110, 0, 0, strlen("Connect Player1 controller"), false);
		}
		//mempak inserted
		else if(identify_accessory(0) != ACCESSORY_MEMPAK){
			if(Down1.A){
				valid = true;
			}
			else{
				DoomCanvas_drawFont(doomCanvas, "No Mempak inserted", 100, 100, 0, 0, strlen("No Mempak inserted"), false);
				DoomCanvas_drawFont(doomCanvas, "Do you play without saving?", 80, 110, 0, 0, strlen("Do you play without saving?"), false);				
				DoomCanvas_drawFont(doomCanvas, "A - Yes", 140, 120, 0, 0, strlen("A - Yes"), false);
			}

		}
		//good mempak
		else if(validate_mempak(0) != 0){
			if(Down1.A) {
				format_mempak(0);
				DoomCanvas_drawFont(doomCanvas, "Formatted", 120, 100, 0, 0, strlen("Formatted"), false);
			}
			else{
				DoomCanvas_drawFont(doomCanvas, "MemPak Unreadable", 100, 100, 0, 0, strlen("MemPak Unreadable"), false);
				DoomCanvas_drawFont(doomCanvas, "Do you want to Format?", 80, 110, 0, 0, strlen("Do you want to Format?"), false);
				DoomCanvas_drawFont(doomCanvas, "A - Yes", 140, 120, 0, 0, strlen("A - Yes"), false);
			}
		}
		else{
			if(Pressed1.start) DoomCanvas_SaveManager(doomCanvas);
			valid = true;
		}
		...
		// cliprect.w = 300;
		// cliprect.h = 220;
		// rdp_set_fill_color(0,0,200,255);
		// SDL_RenderFillRect2(NULL, &cliprect);
...
DoomCanvas_t* DoomCanvas_init(DoomCanvas_t* doomCanvas, DoomRPG_t* doomRpg) // 0x12A20
{
	...
	doomCanvas->slowBlit = SDL_TRUE; //0;
	...
	doomCanvas->Deblur = false;
	doomCanvas->FPS = false;
...
void DoomCanvas_setupmenu(DoomCanvas_t* doomCanvas, boolean notdrawLoading)
{
	if (notdrawLoading == false) {
		...
		//DoomRPG_flushGraphics(doomCanvas->doomRpg); //Fixes solid floors crash and causes crash
	}
...
void DoomCanvas_captureDogState(DoomCanvas_t* doomCanvas)
{
	...
	if (doomCanvas->time >= doomCanvas->captureTime) {
		...
		case 0:
			...
			doomCanvas->captureTime = (doomCanvas->time + 100) % MAXTICKS;
			break;
		case 1:
			...
			doomCanvas->captureTime = (doomCanvas->time + 500) % MAXTICKS;
			break;
		case 2:
...
void DoomCanvas_combatState(DoomCanvas_t* doomCanvas)
{
	...
	Combat_t* combat;
	testsub("DoomCanvas_combatState");
	...
	testsub("combatstate 1");
	// sprintf(test, "time=%x f339=%x", doomCanvas->time, combat->f339c);
	// testsub(test);

	//if (combat->curAttacker == NULL || doomCanvas->time >= combat->f339c) {
	//new
	if (combat->curAttacker == NULL || doomCanvas->time >= combat->f339c 
		|| doomCanvas->time < (combat->f339c - 1000)) {	
		//|| resettics != false){ 		
		testsub("combatstate 1a");
		if (doomCanvas->combatDone) {
			testsub("combatstate 1b");
		...
		testsub("combatstate 1d");
		doomCanvas->doomRpg->hud->isUpdate = true;
	}
	testsub("combatstate 3");
...
void DoomCanvas_dialogState(DoomCanvas_t* doomCanvas)
{
	...
	x1 = doomCanvas->SCR_CX - 65;
	x2 = x1 + 128;
	y1 = doomCanvas->displayRect.h - 54 - 20;
	y2 = y1 + 54;


	DoomRPG_setColor(doomCanvas->doomRpg, 0x000000);
	//DoomRPG_fillRect(doomCanvas->doomRpg, doomCanvas->SCR_CX - 64, doomCanvas->displayRect.h - 54, 128, 54);
	DoomRPG_fillRect(doomCanvas->doomRpg, x1 + 1, y1+1, 128, 54);
	...
	posY = y1 + 2; //doomCanvas->displayRect.h - 52;
	for (i = 0; i < 4 && doomCanvas->currentDialogLine + i < doomCanvas->numDialogLines; ++i) {
		...
		DoomCanvas_drawFont(doomCanvas, doomCanvas->dialogBuffer, x1 + 1, posY, 0, strBeg, strEnd, 0);
	...
	if (doomCanvas->numDialogLines > 4) {
		if (doomCanvas->currentDialogLine + 4 == doomCanvas->numDialogLines) {
			...
			DoomCanvas_drawScrollBar(doomCanvas, y1, 53, doomCanvas->currentDialogLine, doomCanvas->numDialogLines, doomCanvas->numDialogLines);
		}
		else {
			...
			DoomCanvas_drawScrollBar(doomCanvas, y1, 53, doomCanvas->currentDialogLine, doomCanvas->currentDialogLine + 4, doomCanvas->numDialogLines + 4);
		}
	}
}
...
void DoomCanvas_drawAutomap(DoomCanvas_t* doomCanvas, boolean z)
{
	...
	if (z)
	{
		...
		DoomRPG_setColor(doomCanvas->doomRpg, 0xCC0000);
		int i19;
		for (i19 = 0; i19 < doomCanvas->render->linesLength; i19++) {
	...
	if (doomCanvas->time > doomCanvas->automapBlinkTime) {
		doomCanvas->automapBlinkTime = (doomCanvas->time + 333) % MAXTICKS;
	...
	if (py < (doomCanvas->displayRect.y + doomCanvas->displayRect.h)) {
		...
		DoomCanvas_drawTile(doomCanvas, &doomCanvas->imgMapCursor, 0, i25, 6, 6, 0, px, py, 0); //0x30);
	}
}
	...
	if (doomCanvas->castSeq == -1) {
		if (doomCanvas->castTime == 0) {
			...
			if (doomCanvas->time < doomCanvas->castTime 
				&& doomCanvas->time > (doomCanvas->castTime - 2500)) { //new
				DoomRPG_setColor(doomCanvas->doomRpg, 0x000000);
				DoomRPG_fillRect(doomCanvas->doomRpg, 0, 0, doomCanvas->displayRect.w, doomCanvas->displayRect.h);
		...
		if (doomCanvas->time >= (doomCanvas->castTime + 1500)%MAXTICKS) {
		...
		testsub("DoomCanvas_castState 1");
		Render_render(doomCanvas->render, doomCanvas->viewX, doomCanvas->viewY, doomCanvas->viewZ, doomCanvas->viewAngle);
		DoomCanvas_drawRGB(doomCanvas);
		...
		switch (doomCanvas->castSeq) {
			case 0:
				if (doomCanvas->time > (doomCanvas->castTime + 200) % MAXTICKS) {
			...
			case 1:
				if (doomCanvas->time > (doomCanvas->castTime + 100) % MAXTICKS) {
					if (!spectreBlephegor) {
						sprite->renderMode = 5;
					}
					doomCanvas->castSeq++;
			...
			case 4:
				if (doomCanvas->castEntityEndAttackTime == 0) {
					doomCanvas->field_0xdd8 = 0;
					doomCanvas->castEntityLoopFrames = Combat_getMonsterWeaponInfo(doomCanvas->doomRpg->combat, doomCanvas->castEntity->def->eSubType, 0);
					doomCanvas->castEntityBegAttackTime = (Combat_getMonsterWeaponInfo(doomCanvas->doomRpg->combat, doomCanvas->castEntity->def->eSubType, 1) * 10) % MAXTICKS;
					doomCanvas->castEntityEndAttackTime = (doomCanvas->time + doomCanvas->castEntityBegAttackTime) % MAXTICKS;
					if (doomCanvas->field_0xddc != 0) {
						doomCanvas->castEntityEndAttackTime = (doomCanvas->castEntityEndAttackTime + 500) % MAXTICKS;
					}
				}
				...
				if (doomCanvas->time > doomCanvas->castEntityEndAttackTime)
				{
					...
					if (doomCanvas->field_0xdd8) {
						...
						doomCanvas->castEntityEndAttackTime = (doomCanvas->castEntityBegAttackTime + doomCanvas->time) % MAXTICKS;
					}
					else {
						doomCanvas->castEntityLoopFrames--;
						sprite->info &= 0xffffe1ff;
						doomCanvas->castEntityEndAttackTime = ((doomCanvas->castEntityBegAttackTime / 2) + doomCanvas->time) % MAXTICKS;
					}
				}
			...
			case 5:
				if (doomCanvas->time > (doomCanvas->castTime + 500) % MAXTICKS)
				{
					// Death Sound
					...
					if ((unsigned int)(spriteId - 41) <= 2) {
						...
						gSprite->flags |= 4;
						gSprite->time += (DoomRPG_randNextByte(&doomCanvas->doomRpg->random) / 100) % MAXTICKS;
			...
			case 6:
				if (doomCanvas->time > (doomCanvas->castTime + 250) % MAXTICKS)
				{
			...
			case 7:
				if (doomCanvas->time > (doomCanvas->castTime + 400) % MAXTICKS) {
					if (!spectreBlephegor) {
						sprite->renderMode = 6;
					}
					doomCanvas->castSeq++;
			...
			case 11:
				if (doomCanvas->time > (doomCanvas->castTime + 1000) % MAXTICKS)
				{
...
void DoomCanvas_drawRGB(DoomCanvas_t* doomCanvas)
{
	...
	testsub("DoomCanvas_drawRGB");

	//if (doomCanvas->time < doomCanvas->shaketime) {
	if(doomCanvas->shaketime > 0){
	...
	//SDL_UpdateTexture2(sdlVideo.renderer, &clip, doomCanvas->render->framebuffer, sdlVideo.rendererW * 2);
...
void DoomCanvas_drawImageSpecial(DoomCanvas_t* doomCanvas, Image_t* img, int xSrc, int ySrc, int width, int height, int param_7, int xDst, int yDst, int flags)
{
	...
	testsub("DoomCanvas_drawImageSpecial");
	...
	if (img->imgBitmap) {
		...
		do {
			do {
				...
				SDL_RenderCopy2(sdlVideo.renderer, img->imgBitmap, &clip, &renderQuad);
...
void DoomCanvas_drawTile(DoomCanvas_t* doomCanvas, Image_t* img, int xSrc, int ySrc, int width, int height, int param_7, int xDst, int yDst, int flags){
	SDL_DrawTexturedTile(img->imgBitmap->data + (ySrc * (width)), xDst, yDst, img->width-1, height-1, flags);
}

void DoomCanvas_drawStatusBar(DoomCanvas_t* doomCanvas, Image_t* img, int xSrc, int ySrc, int width, int height, int param_7, int xDst, int yDst, int flags){
	SDL_DrawTexturedHTiles(img->imgBitmap, width, img->height, xDst, yDst);
}

void DoomCanvas_drawScrollBar(DoomCanvas_t* doomCanvas, int y, int totalHeight, int i3, int i4, int i5)
{
	testsub("DoomCanvas_drawScrollBar");
	int i6 = i4 - i3;
	if (i6 != 0) {
		...
		DoomCanvas_drawTile(doomCanvas, &doomCanvas->doomRpg->menuSystem->imgArrowUpDown, 0, 0, 7, 7, 0, doomCanvas->SCR_CX + 64 - doomCanvas->doomRpg->menuSystem->imgArrowUpDown.width, y, 0); //9);
		...
		DoomCanvas_drawTile(doomCanvas, &doomCanvas->doomRpg->menuSystem->imgArrowUpDown, 0, 0, 7, 7, 0, doomCanvas->SCR_CX + 64 - doomCanvas->doomRpg->menuSystem->imgArrowUpDown.width, y + totalHeight - 5, 2); //10);
...
{
	int x, y, x1, y1;
	testsub("DoomCanvas_drawSoftKeys");
...
void DoomCanvas_drawFont(DoomCanvas_t* doomCanvas, char* text, int x, int y, int flags, int strBeg, int strEnd, boolean isLargerFont)
{
	...
	for (i = strBeg; i < len; ++i)
	{
		...
		if (c == 10) {
			...
			if (c != ' ') {
				// DoomCanvas_drawImageSpecial(doomCanvas,imgFont,
				// width * ((c - 33) & 0xf),
				...
				DoomCanvas_drawTile(doomCanvas, imgFont, 
					0,//width * ((c - 33) & 0xf), 
					height * ((c - 33)), //height * ((unsigned int)((c - 33) << 24) >> 28),
					width,
					height,
					0,
					xpos,
		...
		testsub("DoomCanvas_drawFont_end");
}
...
void DoomCanvas_dyingState(DoomCanvas_t* doomCanvas)
{
	...
	if (doomCanvas->time < (doomCanvas->deathTime + 750) % MAXTICKS) {
	...
	else if (doomCanvas->time < (doomCanvas->deathTime + 3000) % MAXTICKS) {
...
{
	testsub("DoomCanvas_finishMovement");
...
{
	testsub("DoomCanvas_finishRotation");
...
{
	testsub("DoomCanvas_flagForFacingDir");
...
void DoomCanvas_handleEvent(DoomCanvas_t* doomCanvas, int i) {
	...
	case ST_COMBAT:
		if (doomCanvas->combatDone != 0) {
			...
			Hud_addMessage(doomCanvas->doomRpg->hud, "Calculating Combat");
		}
		break;
...
void DoomCanvas_handlePasswordEvents(DoomCanvas_t* doomCanvas, int i)
{
	...
	if (doomCanvas->passwordTime == 0) {
		...
		if (z) {
			if (len1 == len2) {
				doomCanvas->passwordTime = (doomCanvas->time + 300) % MAXTICKS;
			}
			else {
...
{
	testsub("DoomCanvas_loadPrologueText");
	int textLen;
	DoomRPG_setColor(doomCanvas->doomRpg, 0x000000);
	DoomRPG_fillRect(doomCanvas->doomRpg, 0, 0, doomCanvas->displayRect.w, doomCanvas->displayRect.h);
...
boolean DoomCanvas_loadMedia(DoomCanvas_t* doomCanvas)
{
	...
	if (Render_beginLoadMap(doomCanvas->render, doomCanvas->loadMapID))
	{
		...
		if (Render_beginLoadMapData(doomCanvas->render))
		{
			...
			doomCanvas->isUpdateView = true;
			DoomCanvas_setState(doomCanvas, ST_PLAYING);
			doomCanvas->idleTime = (doomCanvas->time + 8000) % MAXTICKS;
			return true;
		}
	}
...
void DoomCanvas_LoadMenuMap(DoomCanvas_t* doomCanvas)
{
	if (Render_beginLoadMap(doomCanvas->doomRpg->render, MAP_MENU))
	{
		if (Render_beginLoadMapData(doomCanvas->doomRpg->render))
		{
			...
			int i;
			for (i = 0; i < doomCanvas->doomRpg->render->screenWidth; i++) {
...
void DoomCanvas_legalsState(DoomCanvas_t* doomCanvas) {
	...
	testsub("DoomCanvas_legalsState");
		...
		if (game->activeSprites || !game->monstersTurn || doomCanvas->openDoorsCount != 0 || Game_updateMonsters(game))
		{
		  ...
		  //    without player movement updates intervening.
			...
			if (activeMonsters && (doomCanvas->time > doomCanvas->idleTime))
			{
				...
				doomCanvas->idleTime = (doomCanvas->idleTime + doomCanvas->time + 6000) % MAXTICKS;
...
{
	testsub("DoomCanvas_renderScene");
	...
	testsub("DoomCanvas_renderScene_end");
}
...
{
	testsub("DoomCanvas_runInputEvents");
	int i;
...
void DoomCanvas_run(DoomCanvas_t* doomCanvas)
{
	...
	//{
	DoomRPG_setColor(doomCanvas->doomRpg, 0x000000);
	testsub("DoomCanvas_run 1a");

	DoomRPG_clearGraphics(doomCanvas->doomRpg);
	//SDL_RenderCopy(sdlVideo.renderer, &doomCanvas->imgLegals.imgBitmap,);
	// int i,j;
	//     for(i = 0; i < doomCanvas->imgLegals.height; i++){
    // 	for(j = 0; j < doomCanvas->imgLegals.width; j++){
    //             fbuffer[(i * 320) + j] = doomCanvas->imgLegals.imgBitmap[(i * doomCanvas->imgLegals.width) + j];
    //         }
    //     }
	// SDL_RenderPresent(sdlVideo.renderer);

	//}
	testsub("DoomCanvas_run 1b");
	//DoomCanvas_updateLoadingBar(doomCanvas);
		...
		case ST_LOADING: {
			...
			if (doomCanvas->loadType == 0) {
				...
				if(identify_accessory(0) == ACCESSORY_MEMPAK){
					...
					DoomCanvas_drawString1(doomCanvas, "No Controller Pak Inserted", doomCanvas->SCR_CX, doomCanvas->SCR_CY - 24, 17);
					DoomCanvas_drawString1(doomCanvas, "Insert Pak then Save", doomCanvas->SCR_CX, doomCanvas->SCR_CY - 14, 17);
					DoomCanvas_drawString1(doomCanvas, "Press A to Continue", doomCanvas->SCR_CX, doomCanvas->SCR_CY - 4, 17);
					DoomRPG_flushGraphics(doomCanvas->doomRpg); 
					while(1){
						getpad();
						if(Down1.A) break;
	...
	if (doomCanvas->renderOnly && (doomCanvas->state == ST_PLAYING)) {
		if (doomCanvas->lastFrameTime == doomCanvas->time) {
			getstate(state);
			DoomRPG_flushGraphics(doomCanvas->doomRpg);
			//memset(sdlVideo.renderer, 0, (320*240*sizeof(short)));
		}
	}
	else {
		getstate(state);
...
{
	testsub("DoomCanvas_setAnimFrames");
...
void DoomCanvas_setState(DoomCanvas_t* doomCanvas, int stateNum)
{
	...
	sprintf(test, "DoomCanvas_setState statenum=%i\n", stateNum);
	testsub(test);
	...
	if (stateNum == ST_SORRY) {
		...
		DoomRPG_flushGraphics(doomCanvas->doomRpg); //fixes saving crash
	}
	...
	testsub("DoomCanvas_setState_end");
}
...
void DoomCanvas_startShake(DoomCanvas_t* doomCanvas, int i, int i2, int i3)
{
	if (!doomCanvas->slowBlit) {
		if (i2 != 0) {
			doomCanvas->shaketime = i>>3; //doomCanvas->time + i;
...
testsub("DoomCanvas_startup2");
	...
	DoomRPG_createTiledImage(doomCanvas->doomRpg, "a.bmp", true, &doomCanvas->imgFont, 9, 12);
	#ifdef BREW
	//DoomRPG_createImage(doomCanvas->doomRpg, "larger_font.bmp", true, &doomCanvas->imgLargerFont);
	DoomRPG_createTiledImage(doomCanvas->doomRpg, "larger_font.bmp", true, &doomCanvas->imgLargerFont, 13, 17);
	#endif
...
void DoomCanvas_uncoverAutomap(DoomCanvas_t* doomCanvas)
{
	...
	testsub("DoomCanvas_uncoverAutomap");
...
void DoomCanvas_checkFacingEntity(DoomCanvas_t* doomCanvas)
{
	...
	testsub("DoomCanvas_checkFacingEntity");
...
{
	int x, y, i;
	testsub("DoomCanvas_updateLoadingBar");
	...
	//testsub("DoomCanvas_updateLoadingBar end");
}
...
boolean DoomCanvas_updatePlayerAnimDoors(DoomCanvas_t* doomCanvas)
{
	...
	int flags;
	testsub("DoomCanvas_updatePlayerAnimDoors");
	...
	int i;
	for (i = 0; i < doomCanvas->openDoorsCount; i++) {
