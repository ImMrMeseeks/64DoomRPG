...
int Combat_monsterSeq(Combat_t* combat)
{
	...
	switch (combat->stage) {
		case 0:
			testsub("Combat_monsterSeq 0");
			combat->f336b = false;
			combat->stage = 1;
			combat->animEndTime = (doomCanvas->time + 250) % MAXTICKS;
			combat->frameTime = 0;
		...
		case 1:
			...
			if (combat->frameTime == 0) {
				...
				if (doomCanvas->time <= combat->animEndTime
					...
					testsub("Combat_monsterSeq 1b");
					return false;
				}

				if (combat->animLoopCount <= 0 && !combat->f336b) {
					Combat_updateProjectile(combat);
					testsub("Combat_monsterSeq 1c");
					return 0;
				}
				...
				if (combat->f336b) {
					...
					combat->frameTime = (doomCanvas->time + 150) % MAXTICKS; //new % MAXTICKS
				...
				combat->animEndTime %= MAXTICKS; //new
			}

			if (combat->frameTime != 0 && doomCanvas->time > combat->frameTime
				|| combat->frameTime != 0 && doomCanvas->time < (combat->frameTime - 1000)	) {
				...
				Combat_updateProjectile(combat);
				testsub("Combat_monsterSeq 1d");
				return false;
			}
			else {
				Combat_updateProjectile(combat);
				testsub("Combat_monsterSeq 1e");
				return false;
			}

			break;
		case 2:
		//Hud_addMessage(doomCanvas->doomRpg->hud, "combat=2");
			combat->f336b = false;
...
boolean Combat_playerSeq(Combat_t* combat)
{
	...
	if (combat->stage == 0) {
		testsub("Combat_playerseq stage 0");
		combat->f336b = false;
		combat->f340c = false;
		combat->stage = 1;
		combat->animEndTime = (doomCanvas->time+ 250)  % MAXTICKS;
		...
		testsub("Combat_playerseq stage 1");
		if (combat->nextStageTime != 0) {
			testsub("Combat_playerseq stage 1a");
			if (doomCanvas->time > combat->animEndTime
				|| doomCanvas->time < (combat->animEndTime - 1000)
				|| doomCanvas->time < combat->animEndTime - (10 * wpinfo[(combat->attackerWeaponId * 6) + 1])
			) { //new
				testsub("Combat_playerseq stage 1a1");
				if (combat->f336b) {
					if (!doomCanvas->slowBlit) {
						DoomCanvas_updateViewTrue(doomCanvas);
					}
				...
				if (doomCanvas->time > combat->nextStageTime
					...
					|| doomCanvas->time < combat->animEndTime - (10 * wpinfo[(combat->attackerWeaponId * 6) + 1])
						...
						testsub("Combat_playerseq stage 1a2");
		...
		if (combat->frameTime == 0) {
			testsub("Combat_playerseq stage 1b");
			if (doomCanvas->time <= combat->animEndTime) {
				Combat_updateProjectile(combat);
				return false;
			}
			...
			if (!doomCanvas->slowBlit || (combat->f336b && (combat->animLoopCount == wpinfo[(combat->attackerWeaponId * 6) + 0]))) {
				testsub("Combat_playerseq stage 1b1");
				DoomCanvas_updateViewTrue(doomCanvas);
			}

			if (combat->f336b) {
				testsub("Combat_playerseq stage 1b2");
				combat->frameTime = (doomCanvas->time + 150) % MAXTICKS; //new % MAXTICKS
				combat->endFrameTime = (doomCanvas->time + (10 * wpinfo[(combat->attackerWeaponId * 6) + 1])) % MAXTICKS;
				combat->armorDamage = 0;
				combat->damage = 0;
				...
				if (combat->curTarget->def->eType == 1) { // Enemy
					...
					if (combat->hitType != 0) {
						...
						if (CombatEntity_getHealth(&combat->curTarget->monster->ce) - dmgArmor > 0) {
							...
							combat->curTarget->monster->animFrameTime = (doomCanvas->time + 250) % MAXTICKS;
						}
				...
				combat->animEndTime = (doomCanvas->time + (10 * wpinfo[(combat->attackerWeaponId * 6) + 1])) % MAXTICKS;
			}
			else {
				testsub("Combat_playerseq stage 1b3");
				combat->animEndTime = (doomCanvas->time + (5 * wpinfo[(combat->attackerWeaponId * 6) + 1]))  % MAXTICKS;
			}
		}

		if (combat->frameTime != 0 && doomCanvas->time > combat->frameTime
			|| combat->frameTime != 0 && doomCanvas->time < (combat->frameTime - 1000) //new
			) {
			testsub("Combat_playerseq stage 1c");
		...
		testsub("Combat_playerseq stage 2");
		combat->f336b = false;
		if (combat->nextStageTime == 0) {
			testsub("Combat_playerseq stage 2a");
			DoomCanvas_updateViewTrue(doomCanvas);
			if (combat->totalDamage + combat->totalArmorDamage == 0) {
				...
				testsub("Combat_playerseq stage 2b");
				if (combat->curTarget->def->eType == 1) {
					testsub("Combat_playerseq stage 2b1");
					msgBuff = Hud_getMessageBuffer(hud);
					...
					testsub("Combat_playerseq stage 2b2");
					if (combat->curTarget->def->eSubType == 4) {
						msgBuff = Hud_getMessageBuffer(hud);
					...
					testsub("Combat_playerseq stage 2b3");
					Hud_addMessage(hud, "No effect!");
				}
				else {
					testsub("Combat_playerseq stage 2b4");
					Hud_addMessage(hud, "Missed!");
				}
			...
			if (combat->curTarget->def->eType != 1 || CombatEntity_getHealth(&combat->curTarget->monster->ce) <= 0) {
				combat->f339c = (doomCanvas->time + 1000) % MAXTICKS;
				return true;
			}
			combat->nextStageTime = (doomCanvas->time + 1000) % MAXTICKS;
		}
	...
	testsub("Combat_playerseq _end false");
	return false;
}
...
void Combat_spawnBloodParticles(Combat_t* combat, int i, int i2, int i3)
{
	...
	if (combat->curTarget) {
		if (combat->curTarget->monster) {
			combat->curTarget->monster->damageBlendTime = (combat->doomRpg->doomCanvas->time + 100) % MAXTICKS;
		}
	}
