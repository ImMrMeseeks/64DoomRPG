...
void rdp_load_texture2( char *data, uint16_t width, uint16_t height, uint8_t bitdepth, uint8_t hslices, uint8_t vslices, uint8_t format )
{
    if ( !data ) { return; }
	
    // Save cache for managed sprite commands
    cache.width = width - 1;
    cache.height = height - 1;	
    cache.cp_x = hslices;
    cache.cp_y = vslices;
    cache.cp_start = format;	
	
    // Figure out the power of two this sprite fits into
    cache.real_width  = __rdp_round_to_power( width );
    cache.real_height = __rdp_round_to_power( height );
    uint32_t wbits = __rdp_log2( cache.real_width  );
    uint32_t hbits = __rdp_log2( cache.real_height );
    ...
    if ( bitdepth > 1 ) // 16/32bit textures
    {	
        // Point the RDP at the actual sprite data
        __rdp_ringbuffer_queue( 0x3D000000 | ((bitdepth == 2) ? 0x00100000 : 0x00180000) | cache.width );
        __rdp_ringbuffer_queue( (uint32_t)data );
        __rdp_ringbuffer_send();

        // Instruct the RDP to copy the sprite data out
        __rdp_ringbuffer_queue( 0x35000000 | ((bitdepth == 2) ? 0x00100000 : 0x00180000) | ((((cache.real_width  >> 3) + round_amount) << 1) & 0x1FF) << 9 );
        __rdp_ringbuffer_queue( 0x40100 | hbits << 14 | wbits << 4 );
        __rdp_ringbuffer_send();				
    ...
    {	
        // set correct mode
        uint32_t bit_div = (bitdepth == 0) ? 1 : 0;
        uint32_t wide_x = (width >> (1 + bit_div)) - 1;	
        uint32_t math_line = (((cache.real_width  >> 3) + round_amount) & 0x1FF) >> bit_div;
        ...
        // set tile (2/2), texture: set color index and texture bitdepth
        __rdp_ringbuffer_queue( 0x35400000 | bitdepth << 19 | math_line << 9 ); 
        __rdp_ringbuffer_queue( 0x40100 | use_palette << 20 | hbits << 14 | wbits << 4 );
        __rdp_ringbuffer_send();
    }	
}
...
//use tmem as palette to draw filled colour
void rdp_draw_offset_textured_rect(int tx, int ty, int bx, int by, double x_scale, double y_scale, int offset, int flags)
{
    uint16_t s = 0;
    uint16_t t = 0;
    ...
    s += (int)(((double)((offset) << 5)) * (1.0 / x_scale));
...
//create span from loaded texture
void rdp_drawspan(int texcol, int dc_x, int dc_yl, int centery, int dc_yh, int dc_iscale, int dc_texturemid) {
    double actscale = (double)((dc_iscale >> 1) >> 16);

    //rdp_attach_display(_dc);

    //32x32
    uint16_t s = ((int)((texcol >> 1) & 31) << 5); // + ((((dc_texturemid)+((dc_yl - centery) + offset) * dc_iscale) >> FRACBITS) << 5);
    //32x64
    ...
    //32x64
    uint16_t t = ((((dc_texturemid + ((dc_yl - centery) * dc_iscale)) >> (16 + 2)) & 31) << 5); //0;
    ...
    if (dc_yl < 0)
    {
        ...
        t += ((((dc_texturemid + (((dc_yl - centery) * dc_iscale) >> 16)) & 31) << 5) + 20);

        ty = 0;
    }

    //draw texture1
    __rdp_ringbuffer_queue(0xE4000000 | dc_x << 14 | (dc_yh + 20) << 2);
    //__rdp_ringbuffer_queue128(0xE4000000 | dc_x << 14 | (dc_yh +offset) << 2);
    //load texslot1
    __rdp_ringbuffer_queue(dc_x << 14 | ty << 2);
    /* Set up texture position and scaling to 1:1 copy */
    __rdp_ringbuffer_queue(s << 16 | t);
    ...
    //32x64
    __rdp_ringbuffer_queue(((int)(4096) & 0xFFFF) << 16 | (int)(((long long)(1024 << 16) * (long long)dc_iscale) >> 34));   //((int)(1024/ (double)actscale)) //&0xFFFF); //18
