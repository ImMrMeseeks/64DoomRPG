...
static Uint64 basetime = 0; //Uint32
...
//#if 0
			...
			if (buttomID == CONTROLLER_BUTTON_B) {
				key |= AVK_STAR;
			}

			if (buttomID == CONTROLLER_BUTTON_BACK) {
				key |= AVK_PASSTURN;
			}
			if (buttomID == CONTROLLER_BUTTON_LEFT_BUMPER) {

				key |= AVK_PASSTURN;
			}
			if (buttomID == CONTROLLER_BUTTON_RIGHT_BUMPER) {

				key |= AVK_AUTOMAP;
			}

			if(buttomID == CONTROLLER_BUTTON_LAXIS_LEFT){
				key |= AVK_MENU_PAGE_UP;
				key |= AVK_LEFT;
			}
			if(buttomID == CONTROLLER_BUTTON_LAXIS_RIGHT){
				key |= AVK_MENU_PAGE_DOWN;
				key |= AVK_RIGHT;
			}
//#endif
			...
			// Menu page up
			if ((buttomID == CONTROLLER_BUTTON_DPAD_LEFT)) {
				key |= AVK_MENU_PAGE_UP;
				key |= AVK_MOVELEFT;
			}

			// Menu page down
			if ((buttomID == CONTROLLER_BUTTON_DPAD_RIGHT)) {
				key |= AVK_MENU_PAGE_DOWN;
				key |= AVK_MOVERIGHT;
			}
		}
											...
											if (doomRpg->combat)
											{
												...
												if (EntityDef_startup(doomRpg->entityDef))
												{
													if (Render_startup(doomRpg->render))
													{
														...
														//n64 New
														DoomCanvas_CheckPak(doomRpg->doomCanvas);
...
{
	sprintf(test, "DoomRPG_CreateImage=%s\n", resourceName);
	testsub(test);
	...
	rw = SDL_RWFromFile(fileName, &isTransparentMask);
	if (!rw) {
	...
	freedata(loadedSurface->w * loadedSurface->h *sizeof(short));
	...
	doomrpg->imageMemory += (DoomRPG_freeMemory() - mem);
		testsub("DoomRPG_CreateImage end");
}

void DoomRPG_createTiledImage(DoomRPG_t* doomrpg, const char* resourceName, boolean isTransparentMask, Image_t* img, int spriteWidth, int spriteHeight){
	sprintf(test, "DoomRPG_CreateImage=%s\n", resourceName);
	testsub(test);
	...
	//Create texture from surface pixels
	//newTexture = SDL_CreateTextureFromSurface(sdlVideo.renderer, loadedSurface);
	newTexture = SDL_CreateTiledTextureFromSurface(sdlVideo.renderer, loadedSurface, spriteWidth, spriteHeight);
	if (newTexture == NULL) {
		DoomRPG_Error("Unable to create texture from %s! SDL Error: %s\n", fileName, SDL_GetError());
	}
	...
	img->width = spriteWidth; //loadedSurface->w;
	img->height = spriteHeight; //loadedSurface->h * (loadedSurface->w / spriteWidth);
	img->imgBitmap = newTexture;
	//newTexture->w = spriteWidth;
	//Get rid of old loaded surface
	freedata(loadedSurface->w/2 * loadedSurface->h); //new
	SDL_FreeSurface(loadedSurface);

	//SDL_free(fdata);

	 int i;
	 for(i = 0; i < (img->width * spriteHeight); i++){
		printf("tex pos=%x val=%x", i, img->imgBitmap->data[i]);
		testsub("-");
	 }
...
void DoomRPG_createImageBerserkColor(DoomRPG_t* doomrpg, const char* resourceName, boolean isTransparentMask, Image_t* img)
{
	...
	freedata(loadedSurface->w * loadedSurface->h *sizeof(short)); //new
	//Get rid of old loaded surface
	SDL_FreeSurface(loadedSurface);
...
{
	freedata(image->width * image->height * sizeof(short)); //new
	//testsub("DoomRPG_freeImage");
...
{
	sprintf(test, "DoomRPG_fileOpenRead=%s\n", resourceName);
	testsub(test);
	//SDL_RWops* rw;
	byte *fdata;
	int fSize;
	...
	//fdata = readZipFileEntry(fileName, &zipFile, &fSize);
	fdata = readZipFileEntry(resourceName, &zipFile, &fSize);
	return fdata;
}
...
//Uint16 DoomRPG_shortAtNext(byte* data, int* posData)
{
	//printf("DoomRPG_shortAtNext\n");
...
//Uint16 DoomRPG_shortAt(byte* data, int posData)
{
	//printf("DoomRPG_shortAt\n");
	//return (short)((data[posData + 0] << 8) | data[posData + 1]);
	return (short)((data[posData + 1] << 8) | data[posData + 0]);
}
int DoomRPG_intAtNext(byte* data, int* posData)
//Uint32 DoomRPG_intAtNext(byte* data, int* posData)
{
	int pos = *posData + 4;
	*posData = pos;
...
//Uint32 DoomRPG_intAt(byte* data, int posData)
{
	//return (int)((data[posData + 0] << 24) | (data[posData + 1] << 16) | (data[posData + 2] << 8) | data[posData + 4]);
...
void DoomRPG_fillRect(DoomRPG_t* doomrpg, int x, int y, int w, int h)
{
	...
	rect.w = w;
	rect.h = h;
	SDL_RenderFillRect2(sdlVideo.renderer, &rect);
}
...
{
	testsub("File_writeShort");

	Uint16 sData;
	sData = i; //SDL_SwapLE16(i);
	if (rw) {
...
{
	testsub("File_writeInt");

	Uint32 iData;
	iData = i; //SDL_SwapLE32(i);
	if (rw) {
...
{
	testsub("File_writeLong");
	Uint32 lData;
	lData = i; //SDL_SwapLE32(i);
	if (rw) {
...
byte File_readByte(SDL_RWops* rw)
{
	Uint8 bData;
...
Uint16 File_readShort(SDL_RWops* rw)
{
	Uint16 sData;
	...
	return (Uint16)SDL_SwapLE16(sData);
}

Uint32 File_readInt(SDL_RWops* rw)
{
	Uint32 iData;

	if (rw) {
		//SDL_RWread(rw, &iData, sizeof(int), 1);
		iData = (int)GetLong(rw);
		rw+=sizeof(int);
	}

	return (Uint32)SDL_SwapLE32(iData);
}

Uint32 File_readLong(SDL_RWops* rw)
{
	Uint32 lData;
	...
	return (Uint32)SDL_SwapLE32(lData);
}
