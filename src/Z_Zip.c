...
void openZipFile(const char* name, zip_file_t* zipFile)
{
	...
	// 	for (i = n - 4; i > 0; i--)
	// 	{
	// 		if (!SDL_memcmp(((Uint8*)(buf + i)), (Uint8*)"PK\5\6", 4)) {
...
unsigned char* readZipFileEntry(const char* name, zip_file_t* zipFile, int* sizep)
{
	...
	handle = dfs_open(name);
	*sizep = dfs_size(handle);

	sprintf(test, "name=%s handle=%i\n size=%i openfiles=%i", name, handle, *sizep, openfiles);
	testsub(test);

	fdata = SDL_malloc(*sizep);
	dfs_read(fdata, *sizep, 1, handle);
	dfs_close(handle);
